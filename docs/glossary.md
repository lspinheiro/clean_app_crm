...
[staff](#staff) — and is created by an owner's invitation. A **pool membership** makes
the account a cleaner in the company's pool and is created through an invite link. One
account can hold many memberships, of both kinds, across companies — the model never
forces a person to hold two logins.

- **Not:** the account itself (one account holds many memberships), and not a role — the
  role lives on the employee membership.
...
recurring assignments, roster, jobs, dispatch, and the pool. A staff member cannot
