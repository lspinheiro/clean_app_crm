...
the work the person does and wants), years of experience, weekly availability, region
preference, languages, transport, an optional photo, and an optional note. One profile per person — the person maintains it once, and
every [company](#company) the person requests to join or works for sees the same
profile.

- **Not:** a per-company record (companies read the profile; they do not own a copy),
  and not the vetting record (badges and checks are F4, MVP).
- **See:** [Join request](#join-request) · [Cleaner staff](#cleaner-staff) ·
...
An [owner](#owner)'s invitation for one e-mail address to hold an employee
[membership](#membership) in their [company](#company). It has exactly one lifecycle
state, and every reader — the owner's list, the invitee's page before and after sign-in,
and the guards behind acceptance and revocation — says the same word:

- **Pending** — live and usable. The only state that can be accepted or revoked.
- **Accepted** — the invitee took it up; the membership exists.
- **Revoked** — the company withdrew it.
- **Replaced** — a newer invitation for the same company and address took its place. An
  owner re-inviting a lapsed address produces this; the older invitation is replaced, not
  withdrawn and not merely expired, and the invitee is told to open the newer e-mail.
- **Expired** — the seven days ran out and nobody re-invited.

- **Not:** a *cleaner invitation* (a link that opens a [join request](#join-request); an
  employee invitation is addressed to one person and grants a company-side role instead).
  Never say an invitation was "superseded" in the interface — `superseded_at` is the
  column, *replaced* is the word. Replaced outranks expiry: a replaced invitation is
  never shown as expired, even though re-inviting only ever replaces a lapsed one.
- **See:** [Membership](#membership) · [Owner](#owner) · [Company admin](#company-admin).

## General thread

The persistent conversation between one [company](#company) and one person. It opens
when the person's [join request](#join-request) is created, so candidacy questions are
its first content. It survives admission and carries ad-hoc and relationship talk
between the company and the [cleaner](#cleaner-staff). Rejection of the join request
makes it read-only.

- **Not:** a [job thread](#job-thread) (talk scoped to one job), and not a group chat —
  each relationship has exactly one general thread.
...
The conversation scoped to one [job](#job) and one assigned cleaner. It opens when the
crew slot is assigned; the participants are that cleaner and the company's admins. It
leads with the job context (site, service, time, status, crew slot), shows the job's
structured field events in its timeline, and goes read-only a few days after the job
completes. A cancelled job or a released slot also closes its thread. On a crew job,
each assigned cleaner has a separate job thread.

- **Not:** the [general thread](#general-thread) (relationship talk), not a crew group
  chat (crew mates never share a thread), and not a pre-assignment channel — applicants
  and pending-offer recipients use the general thread.
- **See:** [General thread](#general-thread) · [Job](#job) · [Crew slot](#crew-slot).
