...
The Clean Crew is a pnpm monorepo for a commercial-cleaning system of record. The working implementation includes a bilingual Next.js company-admin CRM, a client-first/static-exportable Next.js cleaner app, and a Supabase-backed data package. Current slices cover one-off jobs and crew slots, cleaner invitation/pool joining and the open-jobs board, client/site CSV import, and a read-only company pay ledger. The CRM supports Australian English (`en-AU`) and Brazilian Portuguese (`pt-BR`) through locale-prefixed routes and a persistent authenticated preference; the cleaner app's F15 implementation is not evidenced here. Product requirements in `docs/PRODUCT.md` are canonical direction, while source and tests establish current behaviour.
...
- [Cleaner app workflow](workflows/cleaner-app.md) covers invitation/pool joining, the client-side access gate, the privacy-minimized open-jobs board, and its database contracts.
...
| Change cleaner join, access, or open-jobs board | [Cleaner app workflow](workflows/cleaner-app.md) | `apps/cleaner/src/app/{join,(cleaner)/board}/`; `apps/cleaner/src/lib/auth/use-cleaner.ts` | `JoinScreen`, `useCleaner`, `toVacancies`, `cleaner_invite_preview`, `join_company_pool` | cleaner join/board unit tests; `cle-19-join.spec.ts`; `cle-20-board.spec.ts`; relevant SQL test | `pnpm --filter cleaner test:run -- src/features/board/model.test.ts` |
...
- CRM language selection is explicit; it changes locale-prefixed presentation, never the Australia/Brisbane schedule or AUD operating contract, and does not auto-translate user-authored content. See [bilingual CRM routing](workflows/crm-localization.md#product-boundary-and-invariants).

## Backlog: evidence-blocked documentation

- F15 requires bilingual first-party alpha surfaces, but this update provides CRM evidence only; no cleaner-app locale configuration, catalogs, or preference flow was changed. `docs/PRODUCT.md` F15 and `apps/cleaner/` are the source anchors.
...
- Job-type preference ordering and a cross-pool cleaner weekly agenda are product requirements; the current cleaner consumer implements pool joining and an open-jobs board, not either feature. `docs/PRODUCT.md` F5/F11 is the source anchor.
