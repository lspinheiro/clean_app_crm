...
- [Cleaner app workflow](workflows/cleaner-app.md) covers bilingual localized/legacy routing, invitation joining, the client-side access gate, the privacy-minimized open-jobs application board, assignment-gated My Jobs operations, and database contracts.
...
| Change cleaner locale routes, join, access, board applications, or My Jobs | [Cleaner app workflow](workflows/cleaner-app.md) | `apps/cleaner/src/app/(localized)/[locale]/`; `src/i18n/config.ts`; `src/features/{board,my-jobs}/` | `JoinScreen`, `useCleaner`, `LegacyLocaleRedirect`, `apply_to_job`, `withdraw_application`, `get_cleaner_job_access`, `update_job_status` | feature/component tests; `cle-19-join.spec.ts`; `cle-20-board.spec.ts`; `cle-21-apply.spec.ts`; `cle-24-my-jobs.spec.ts`; `f15-cleaner-i18n.spec.ts` | `pnpm --filter cleaner test:run -- src/features/board/application.test.ts` |
...
<!-- openwiki: broken internal link [workflows/company-membership.md#invariants-and-change-rules] heading anchor "invariants-and-change-rules" does not exist in "workflows/company-membership.md". Fix the href or restore the target, then delete this comment. -->
- CRM context is a selected approved active membership. Owner-only team mutations preserve at least one active owner; see [company membership](workflows/company-membership.md#invariants-and-change-rules).
