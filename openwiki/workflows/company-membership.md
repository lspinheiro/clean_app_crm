...
Settings separates invitation delivery from membership changes. `createEmployeeInvitationAction` in `apps/crm/src/app/actions/employee-invitations.ts` requires an owner, prepares the database invitation, and sends localized delivery through `sendResendEmailBatches`; the acceptance route completes the membership-side flow. Existing-account invitations use a sign-in/acceptance link, while new accounts use the Auth confirmation path. Failed delivery is explicitly handled by revoking the prepared invitation rather than presenting an invite as usable.
...
| Employee invitation composition, delivery, or acceptance | `app/actions/employee-invitations.ts`, `app/[locale]/(auth)/invite/accept/`, `lib/resend.ts` | Owner authorization; locale-aware link generation; delivery failure does not leave a live invitation. | `pnpm --filter crm test:run -- src/app/actions/employee-invitations.test.ts`; conditional journey check: `pnpm crm test:e2e -- cle-83-employee-invitations.spec.ts`. |
